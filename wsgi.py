...
async def mock(ctx):
    ...
    if messages:
        ...
        msg = await ctx.send(mock_text, reference=mocked_message)
...
async def matchups(ctx):
    try:
        ...
        msg = await ctx.send(matchups)
    except KeyError:
        msg = await ctx.send("Could not retrieve matchups. This could be due to the ESPN API failing to return season data.")

@bot.command(name="scores", brief="Sends the scores for the current week")
async def scores(ctx):
    try:
        scores=commander.get_scoreboard_short()
        msg = await ctx.send(scores)
    except KeyError:
        msg = await ctx.send("Could not retrieve scores. This could be due to the ESPN API failing to return season data.")

@bot.command(name="final", brief="Final scores for the previous week")
async def final(ctx):
    try:
        final_scores=commander.get_final()
        msg = await ctx.send(final_scores)
    except KeyError:
        msg = await ctx.send(f"Could not retrieve final scores. This could be due to the ESPN API failing to return season data.")
...
async def projections(ctx):
    try:
        ...
        msg = await ctx.send(projections)
    except KeyError:
        msg = ctx.send(f"Could not retrieve projections. This could be due to the ESPN API failing to return season data.")
...
async def standings(ctx):
    try:
        ...
        msg = await ctx.send(f"Could not retrieve standings. This could be due to the ESPN API failing to return season data.")
